<p id="kyoko" align="center">
<img src="https://i.ibb.co/syy1P6g/kyoko.webp" width="160"/><br/>
<b>歳納京子</b><br/>
Toshino Kyoko<br/>
<img src="https://img.shields.io/badge/LICENE-WTFPL-ebcb8b?style=flat-square"/> <img src="https://img.shields.io/badge/VERSION-1.0.1-a3be8c?style=flat-square"/><br/>
Dokumentasi Publik API Gw<br/>
Bebas diakses kapan aja, dimana aja, gratis, tanpa API Key!<br/>
<u><i>https://api.rei.my.id</i></u>
<br/>
Didukung oleh<br/>
<img src="https://img.shields.io/badge/GO-1.20.10-88C0D0?style=flat-square"/> <img src="https://img.shields.io/badge/FIBER-2.52.2-81A1C1?style=flat-square"/> <img src="https://img.shields.io/badge/FASTHTTP-1.52.0-5E81AC?style=flat-square"/>
<br/>
Dukung gw<br/>
<a href="https://paypal.me/elliottophellia"><img src="https://img.shields.io/badge/PAYPAL-5E81AC?style=flat-square"/></a> <a href="https://saweria.co/elliottophellia"><img src="https://img.shields.io/badge/SAWERIA-EBCB8B?style=flat-square"/></a> <a href="https://ko-fi.com/elliottophellia"><img src="https://img.shields.io/badge/KOFI-D08770?style=flat-square"/></a> <a href="https://trakteer.id/elliottophellia"><img src="https://img.shields.io/badge/TRAKTEER-BF616A?style=flat-square"/></a>
<br/>
Terjemahan<br/>
<a href="https://api.rei.my.id/"><img src="https://img.shields.io/badge/ENGLISH-555555.svg?logo=data:image/svg%2bxml;base64,&style=flat-square"/></a>
<br/>
</p>


## Daftar Isi

<!-- TOC START -->

- [Toshino Kyoko](#kyoko)
  - [Terjemahan](#kyoko)
  - [Daftar Isi](#daftar-isi)
  - [Daftar API](#daftar-api)
    - [ANIME](#anime)
    - [OSINT](#anime)
  - [Lisensi](#lisensi)

<!-- TOC END -->

## Daftar API

<!-- API START -->

### ANIME

| Nama API                   | Dokumentasi                                                                                          | Status | Deskripsi                                                 | Autentikasi    | Limitasi    |
| -------------------------- | ---------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------- | -------------- | ----------- |
| Waifu Generator            | [Link](https://api.rei.my.id/docs/ANIME/WAIFU-Generator/README.id) | ✅     | Dapetin gambar anime SFW dan NSFW.                      | `none`         | 50/1min     |

### OSINT

| Nama API                   | Dokumentasi                                                                                         | Status | Deskripsi                                                 | Autentikasi    | Limitasi    |
| -------------------------- | --------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------- | -------------- | ----------- |
| IP Address Lookup          | [Link](https://api.rei.my.id/docs/OSINT/IP-Address-Lookup/README.id) | ✅     | Cari informasi lengkap tentang alamat IP.              | `none`        | 50/1min     |
| Reverse IP Lookup          | [Link](https://api.rei.my.id/docs/OSINT/Reverse-IP-Lookup/README.id) | ✅     | Cari nama domain dari alamat IP.                       | `none`        | 50/1min     |
| BIN Lookup                 | [Link](https://api.rei.my.id/docs/OSINT/BIN-Lookup/README.id)        | ✅     | Cari informasi lengkap tentang nomor BIN.              | `none`        | 50/1min     |

<!-- API END -->

## Lisensi

```
           DO WHAT THE FUCK YOU WANT TO PUBLIC LICENSE
                   Version 2, December 2024
 
Copyright (C) 2024 Reidho Satria <me@rei.my.id>

Everyone is permitted to copy and distribute verbatim or modified
copies of this license document, and changing it is allowed as long
as the name is changed.
 
           DO WHAT THE FUCK YOU WANT TO PUBLIC LICENSE
  TERMS AND CONDITIONS FOR COPYING, DISTRIBUTION AND MODIFICATION

 0. You just DO WHAT THE FUCK YOU WANT TO.
```